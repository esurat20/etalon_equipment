defmodule EtalonEquipmentWeb.CategoryController do
  use EtalonEquipmentWeb, :controller

  alias EtalonEquipment.Categories
  ...
  def create(conn, %{"category" => category_params}) do
    case Categories.create_category(category_params) do
      {:ok, category} ->
        conn
        |> put_flash(:info, "Категория успешно создана.")
  ...
  def update(conn, %{"id" => id, "category" => category_params}) do
    ...
    case Categories.update_category(category, category_params) do
      {:ok, category} ->
        conn
        |> put_flash(:info, "Категория успешно обновлена.")
        |> redirect(to: category_path(conn, :show, category))
  ...
  def delete(conn, %{"id" => id}) do
    ...
    conn
    |> put_flash(:info, "Категория успешно удалена.")
